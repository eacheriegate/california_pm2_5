...
sd_gdf = all_data[all_data["County"].str.lower() == "san diego"].copy()

# Roads
roads = gpd.read_file(road_path).to_crs(epsg=3310)
roads = roads[roads["RTTYP"].isin(["I", "U", "S", "M", "C"])]
sd_union = sd_gdf.geometry.union_all.buffer(0)
roads_clipped = clip(roads, sd_union)

# Quantile Bins
n_bins = 6
sd_gdf["bin"], bin_edges = pd.qcut(sd_gdf[field], q=n_bins, labels=False, retbins=True, duplicates='drop')
...
sd_gdf.plot(column="bin", cmap=cmap, linewidth=0, ax=ax, edgecolor='none', antialiased=False)
roads_clipped.plot(ax=ax, color="white", linewidth=0.3, alpha=0.6)

ax.set_xlim(*sd_gdf.total_bounds[[0, 2]])
ax.set_ylim(*sd_gdf.total_bounds[[1, 3]])
ax.axis('off')

plt.tight_layout()
os.makedirs("results", exist_ok=True)
plt.savefig("results/pm25_la_map.png", dpi=600, bbox_inches="tight", facecolor='none')
plt.show()
...
plt.tight_layout()
plt.savefig("results/pm25_sd_leg.png", dpi=600, bbox_inches="tight", facecolor='none')
plt.show()