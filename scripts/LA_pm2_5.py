...
gdf = gdf[gdf["County"].str.lower() == "los angeles"].copy()

# Roads
roads = gpd.read_file(road_path).to_crs(epsg=3310)
roads = roads[roads["RTTYP"].isin(["I", "U", "S", "M", "C"])]
la_union = gdf.geometry.union_all.buffer(0)
roads_clipped = clip(roads, la_union)

# Quantile Bins
n_bins = 6
gdf["bin"], bin_edges = pd.qcut(gdf[field], q=n_bins, labels=False, retbins=True, duplicates='drop')
...
gdf.plot(column="bin", cmap=cmap, linewidth=0, ax=ax, edgecolor='none', antialiased=False)
...
plt.tight_layout()
plt.savefig("results/pm25_la_leg.png", dpi=600, bbox_inches="tight", facecolor='none')
plt.show()